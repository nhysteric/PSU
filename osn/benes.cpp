...
void Benes::DFS(int idx, int route)
{
    ...
    while (!st.empty()) {
        ...
        if (path[pr.first ^ 1] < 0) // if the next item in the vertical array is unassigned
            st.push({ pr.first ^ 1,
                      pr.second ^ 1 }); /// the next item is always assigned the opposite of this
                                        /// item, unless it was part of path/cycle of previous node
...
void Benes::gen_benes_route(
    int n, int lvl_p, int perm_idx, const vector<int> &src, const vector<int> &dest)
{
    int levels, i, j, x, s;
    vector<int> bottom1;
    vector<int> top1;
    ...
    if (values == 3) {
        ...
        // std::cout<<"based case 3:"<<switched[lvl_p][perm_idx]<<"
        // "<<switched[lvl_p+1][perm_idx]<<"
...
void Benes::gen_benes_masked_evaluate(
    int n,
    int lvl_p,
    int perm_idx,
    vector<oc::block> &src,
